...
Selecting a catalog row only previews it. With a current-spec row selected, clicking **AUTO-SPEND: OFF** now saves that row as the leveling target and turns auto-spend on in one action; a failed target validation leaves auto-spend off and shows the error. The older **USE FOR LEVELING** action still sets a target without enabling automatic spending.
