...
Middle-click the live button to open or close the saved settings panel without casting. Development plans and their current state are tracked in the [Comic Workboard](workboard/index.html).
...
### Pause Detection
Automatically pauses interception during vehicles, skyriding, and unsupported override bars. Normal ground mounts remain interceptable when Auto-Dismount is enabled; otherwise mounting pauses interception. The icon dims lightly, shows a compact pause mark, and displays the bare reason beneath it (for example, “Skyriding”).
...
**Auto-Spend** adds points to the current allocation without changing specialization. It waits during combat, while another build is importing, or while you have uncommitted talent edits. Conflicting learned talents pause the assistant with a status message unless **Auto-Rebuild on Mismatch** is also on. That separate per-character/spec switch resets the active talent tree and reconstructs both class and specialization allocations, plus hero talents only when unlocked, using the points currently available. Changed client builds, tree hashes, or target exports require reselecting the target. Failed or unconfirmed commits stop the pass; review pending talents before toggling auto-spend to retry.
